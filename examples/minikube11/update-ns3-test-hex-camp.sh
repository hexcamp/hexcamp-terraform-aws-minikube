...
AWS_IP=$(aws route53 list-resource-record-sets --hosted-zone-id $HOSTED_ZONE_ID | jq -r '.ResourceRecordSets[] | select(.Name == "ns3.test.hex.camp.").ResourceRecords[0].Value')
